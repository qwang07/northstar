...
- **回路收敛阀**：任一回边对**同一缺口**反复触发达阈值（默认 3 次）仍不收敛 → BLOCKED，停自动回踢、升级给人决策，附该缺口回踢历史（过程态，进 git/GitHub Issues，不入契约）。**计数只计本轮引入的缺口**——既有债经出境记录，不参与门控计数。
...
- **README 唯一写入口**：全回路只有 brainstorming 可写 README；**其余各相一律只读**，发现缺口回踢。配套：一处权威 + 指针，禁止跨 README 复述。
