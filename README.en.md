...
├── tests/pressure/                     acceptance gates for the dogma itself: naive-subagent pressure test for the binding-layer notes + A/B control method for clause load-bearing
├── README.md
└── README.en.md
```
...
| `brainstorming` | contract · sole README write entry | open with the goal, interrogate intent to completeness (one question at a time); distill existing code before asking to adjust; business-flow walkthrough before finalizing; close out when all modules are done | README clauses landed by goal: project-level (module list + topology [each edge carries an interface contract] + boundary interfaces + **delivery result** [form / landing place / how it is consumed / version marker], **no internals**) / cross-cutting layer (constraints shared by multiple modules, on demand) / module-level (responsibility & boundary / expected features / interface contract / edge cases & invariants, **internals only**) |
...
- **Non-functional constraints follow the same "declare → derive → review" path**: security and performance get **no phase of their own** — they enter the loop by being **declared at a cross-cutting layer**, are derived into cases by write-test (security = negative cases from an attacker's viewpoint; performance = the contract carries its own threshold and measurement terms), and are checked by code-review as a review dimension. The declaration is the only switch: what the contract does not declare is neither derived nor reviewed; **something declared but with no derivation and no review dimension is a variant of the always-true assertion** — exactly what the strong-assertion discipline exists to kill.
