...
**派发工具参数（实测 2026-08）**：`description`（必填，短标签）· `prompt`（必填，人格与任务全文，按对应 SKILL.md 现场组装）· `subagent_type`（省略即通用子代理类型；本 plugin 恒用默认，不指定）· `model` · `isolation`（省略即不隔离，共用当前工作树；并行编排时取 `worktree`）· `run_in_background`（布尔；单执行者派发不得开启——须阻塞等待拿回结果才能继续回路。省略时的默认值未实测，故显式传关闭，不倚赖默认）。下表右列的取值直接作为参数值传入，`model` 用档位简称即可，不需要完整型号标识。
...
| 工具面 = 读写 | 默认，无需额外参数 |
| 工具面 = 只读 | **本平台派发工具不接受工具面参数**——以提示词明令 + 返回后基线比对兑现，见下节 |
...
**范围界定（其一·参数类别）**：这里不成立的只有**工具面**这一类参数；上节列出的 `model` / `isolation` / `run_in_background` 是派发工具接受的真实参数，不受此限——两处不矛盾。

**范围界定（其二·通道）**：上述否定限于**原生派发工具**，**不得外推为「本平台一律无法事前限定工具面」**。经外部执行代理插件把活转派给另一引擎时，该通道可能自带事前沙箱参数（2026-08 实测存在这样的通道）。那是**附加，不是替代**：其粒度为**工作区级**——挡得住"写到仓库外"，挡不住"改仓库内的契约与测试"；且此类通道往往默认可写、默认无人工批准闸。故**事后核验三步一律不免除**，禁区仍须在提示词里逐条明令。具体参数以该通道自身文档为准，本文不复述——复述即第二来源，且随其版本漂移。

核验三步：

1. **派发前记基线**：`git status --porcelain`（含未跟踪）+ `git rev-parse HEAD`。
2. **提示词明令**：只读——不得编辑 / 写入 / 移动任何文件，不得动工作树、暂存区与 HEAD；要看其他版本另开 worktree。**需要落草稿、日志或中间产物，写到仓库之外的临时目录**，不得落进工作树。
