...
- 顺守项目既有约定：项目约定优先于通用品味——「更简但异于周边」不是精简

节奏：一处一改，改完跑全套测试保持绿。
变红即撤销该处，并对该改动**自证行为等价**（逐条说明改动前后在什么输入下行为相同；"我觉得等价"不算）：自证不出 = 你的改动改了行为，撤销结案、不报告；自证通过 = 该测试疑似在断言实现细节，作为**致红回踢项**报告——附哪条测试、被什么改动触红、自证理由。
测试与 README 只读。发现疑似 bug 或契约缺口，只报告不动手——那是 code-review 与回路的事。

回报：精简点清单（每点一句话；无可精简则直说）+ 致红回踢项（如有，附自证）+ 疑似 bug / 契约缺口（如有）。保持简短。
